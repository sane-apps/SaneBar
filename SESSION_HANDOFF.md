...
- 2026-05-11 post-`2.1.51` `#142` follow-up is fixed locally for the next patch release:
  ...
  - Release QA hardening expanded after the SaneClick-style miss: `Tests/CustomerUIActions.yml` now lists 20 release-required customer-facing action families instead of 7 broad smoke buckets. Added `Tests/CustomerUIActionContractXCTests.swift` so the contract cannot silently drop status menu, Dock menu, Browse Icons, Second Menu Bar, settings tabs, onboarding, license/about/support, automation, import/export/reset, Health, Pro/Basic gates, or recovery coverage.
  - Added Mini-only `Scripts/customer_ui_action_sweep.rb`. The sweep drives Settings tabs through real AX sidebar selection, captures per-tab settings screenshots, verifies `sanebar://settings`, `sanebar://health`, `sanebar://search?q=Sane`, runs AppleScript layout/list/open/search/panel/settings diagnostics, checks fresh runtime smoke/startup artifacts, then writes `.sane/customer_ui_action_receipt.json`.
  - Current release preflight state: screenshot-enabled Mini `SANEBAR_RELEASE_SMOKE_SCREENSHOTS=1 ./scripts/SaneMaster.rb release_preflight` reaches `Customer UI action contract... ✅ 20 action(s)` and is blocked only by the fast-release cadence guard requiring the exact manual approval phrase `MR. SANE APPROVES FAST RELEASE`. Final Mini verify after the sweep script passed `1158` tests.
...
## 2026-04-23 Arrangement Release Patch

### Done
- Restored the issue #136 arrangement patch after confirming both Air and Mini were clean at the prior 2.1.44 release commit.
- Stopped blocking-mode separator estimates from being stored as trusted separator cache.
- Added wake/screen validation behavior that waits for a live separator anchor when the separator is only estimated but the main anchor is live.
- Fixed layout snapshot `mainRightGap` to fall back from stale status-item frames to the cached main anchor.
- Added real tests for the new coordinator decisions and snapshot gap calculations.

### Verification
- Mini focused `xcodebuild` passed for `MenuBarOperationCoordinatorTests`, `AppleScriptCommandsTests`, and `RuntimeGuardXCTests`.
- Mini `./scripts/SaneMaster.rb verify` passed `1114` tests.
- Mini signed `./scripts/SaneMaster.rb test_mode --release --no-logs` staged `/Applications/SaneBar.app`.
- Mini startup and wake layout probes passed.
- Mini `SANEBAR_RUN_RUNTIME_SMOKE=1 ./scripts/qa.rb` passed runtime smoke x2 plus Siri/Spotlight exact-id smoke; warnings were existing Swift helper-script parse warnings only.
