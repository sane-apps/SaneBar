...
final class SearchServiceProtocolMock: SearchServiceProtocol, @unchecked Sendable {
    ...
    private let activateState = MockoloMutex(
        MockoloHandlerState<
            ...
            @Sendable (RunningApp, Bool, SearchServiceSupport.ActivationOrigin) async -> Void
        >()
    )
    ...
    var activateArgValues: [(RunningApp, Bool, SearchServiceSupport.ActivationOrigin)] {
        activateState.withLock(\.argValues).map(\.value)
    }

    var activateHandler: (@Sendable (RunningApp, Bool, SearchServiceSupport.ActivationOrigin) async -> Void)? {
    ...
    func activate(app: RunningApp, isRightClick: Bool, origin: SearchServiceSupport.ActivationOrigin) async {
        warnIfNotSendable(app, isRightClick, origin)
        let activateHandler = activateState.withLock { state in
            state.callCount += 1
            state.argValues.append(.init((app, isRightClick, origin)))
            return state.handler
        }
        if let activateHandler {
            await activateHandler(app, isRightClick, origin)
        }
    }
}
...
final class HoverServiceProtocolMock: HoverServiceProtocol {
    ...
    private(set) var beginMainStatusItemHoverDwellCallCount = 0
    var beginMainStatusItemHoverDwellHandler: (() -> Void)?
    func beginMainStatusItemHoverDwell() {
        beginMainStatusItemHoverDwellCallCount += 1
        if let beginMainStatusItemHoverDwellHandler {
            beginMainStatusItemHoverDwellHandler()
        }
    }

    private(set) var cancelMainStatusItemHoverDwellCallCount = 0
    var cancelMainStatusItemHoverDwellHandler: (() -> Void)?
    func cancelMainStatusItemHoverDwell() {
        cancelMainStatusItemHoverDwellCallCount += 1
        if let cancelMainStatusItemHoverDwellHandler {
            cancelMainStatusItemHoverDwellHandler()
        }
    }
}
...
final class SettingsControllerProtocolMock: SettingsControllerProtocol {
    ...
    private var _settings: SaneBarSettings! {
        didSet { settingsSetCallCount += 1 }
    }
    ...
    var settingsPublisher: AnyPublisher<SaneBarSettings, Never> {
        settingsPublisherSubject.eraseToAnyPublisher()
    }
