...
struct StatusBarControllerDisplayBackupTests {
    ...
    func ordinalsNotPixelLike(_ value: Double) {
        #expect(!StatusBarPositionStore.isPixelLikePosition(value))
    }

    @Test("AH sentinel (10000) is not pixel-like")
    func ahSentinelNotPixelLike() {
        #expect(!StatusBarPositionStore.isPixelLikePosition(10000))
    }

    @Test("nil is not pixel-like")
    func nilNotPixelLike() {
        #expect(!StatusBarPositionStore.isPixelLikePosition(nil))
    }
    ...
    func sameWidthNotSignificant() {
        #expect(!StatusBarPositionStore.isSignificantWidthChange(stored: 1440, current: 1440))
    }

    @Test("Small width change (<10%) is not significant")
    func smallChangeNotSignificant() {
        // 5% change: 1440 → 1512
        #expect(!StatusBarPositionStore.isSignificantWidthChange(stored: 1440, current: 1512))
    }

    @Test("Large width change (>10%) is significant")
    func largeChangeIsSignificant() {
        // 1440 → 2560 (78% change)
        #expect(StatusBarPositionStore.isSignificantWidthChange(stored: 1440, current: 2560))
    }

    @Test("Zero stored width is not a significant change")
    func zeroStoredNotSignificant() {
        #expect(!StatusBarPositionStore.isSignificantWidthChange(stored: 0, current: 1440))
    }

    @Test("Boundary: exactly 10% change is not significant")
    func boundaryTenPercent() {
        // Exactly 10%: 1000 → 1100
        #expect(!StatusBarPositionStore.isSignificantWidthChange(stored: 1000, current: 1100))
    }

    @Test("Just over 10% change is significant")
    func justOverTenPercent() {
        // 10.1%: 1000 → 1101
        #expect(StatusBarPositionStore.isSignificantWidthChange(stored: 1000, current: 1101))
    }
    ...
    func shouldResetForDisplayChangeSingleDisplay() {
        let shouldReset = StatusBarPositionStore.shouldResetForDisplayChange(
            storedWidth: 1440,
            currentWidth: 2560,
    ...
    func displayBackupWidthBuckets() {
        #expect(StatusBarPositionStore.displayWidthBucket(2559.6) == 2560)
        #expect(StatusBarPositionStore.displayWidthBucket(1439.2) == 1439)
    }

    @Test("Display backup keys are width-bucketed")
    func displayBackupKeyUsesBucket() {
        let keyA = StatusBarPositionStore.displayPositionBackupKey(for: 2559.6, slot: "main")
        let keyB = StatusBarPositionStore.displayPositionBackupKey(for: 2560.2, slot: "main")
        let separatorKey = StatusBarPositionStore.displayPositionBackupKey(for: 2559.6, slot: "separator")
    ...
    func displayBackupScopedKeysSeparateSameWidthScreens() {
        let keyA = StatusBarPositionStore.displayPositionBackupKey(
            for: 1728,
            screenSignature: "d111-h1117-plain",
            slot: "main"
        )
        let keyB = StatusBarPositionStore.displayPositionBackupKey(
            for: 1728,
        ...
        #expect(keyA != keyB)
        #expect(
            !StatusBarPositionStore.shouldAllowLegacyDisplayBackupFallback(
                widthBucket: 1728,
    ...
    func displayBackupRestoreRequiresPixelValues() {
        #expect(StatusBarPositionStore.hasRestorableDisplayBackup(mainBackup: 420, separatorBackup: 840))
        #expect(!StatusBarPositionStore.hasRestorableDisplayBackup(mainBackup: 1, separatorBackup: 840))
        #expect(!StatusBarPositionStore.hasRestorableDisplayBackup(mainBackup: 420, separatorBackup: nil))
    }

    @Test("Launch-safe display backup rejects far-left main positions")
    func launchSafeDisplayBackupRejectsFarLeftMainPositions() {
        #expect(
            StatusBarPositionStore.isLaunchSafeDisplayBackup(
                mainBackup: 180,
                separatorBackup: 220,
                screenWidth: 1470,
    ...
    func reanchoredPreferredPositionsPreserveLaneWidth() {
        let reanchored = StatusBarPositionStore.reanchoredPreferredPositionsTowardControlCenter(
            mainPosition: 456,
        ...
        #expect(
            reanchored?.main == StatusBarPositionStore.launchSafePreferredMainPositionLimit(
                for: 1920,
                screenHasTopSafeAreaInset: false
            )
        )
        ...
        let clamped = StatusBarPositionStore.reanchoredPreferredPositionsTowardControlCenter(
            mainPosition: 456,
    ...
    func captureLayoutSnapshotReadsPersistedState() {
        let originalSnapshot = StatusBarLayoutSnapshotStore.captureLayoutSnapshot()
        defer {
            StatusBarLayoutSnapshotStore.applyLayoutSnapshot(originalSnapshot)
        }

        StatusBarLayoutSnapshotStore.applyLayoutSnapshot(
    ...
    func applyLayoutSnapshotWritesPersistedState() {
        ...
        let staleBackupMainKey = StatusBarPositionStore.displayPositionBackupKey(for: 1440, slot: "main")
        let staleBackupSeparatorKey = StatusBarPositionStore.displayPositionBackupKey(for: 1440, slot: "separator")
        let freshBackupMainKey = StatusBarPositionStore.displayPositionBackupKey(for: 1512, slot: "main")
        let freshBackupSeparatorKey = StatusBarPositionStore.displayPositionBackupKey(for: 1512, slot: "separator")
    ...
    func applyLayoutSnapshotDropsImpossibleDisplayBackups() {
        let defaults = UserDefaults.standard
        let invalidBackupMainKey = StatusBarPositionStore.displayPositionBackupKey(for: 1920, slot: "main")
        let invalidBackupSeparatorKey = StatusBarPositionStore.displayPositionBackupKey(for: 1920, slot: "separator")
